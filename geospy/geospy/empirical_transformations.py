...
# This program transform a set of coordinates from two different referencing 
...
def affineToxy(X,Y,a,b):
    """
    This function is the inverse of xyToAffineRaw
    """
    a0 = a['a0']
    a1 = a['a1']
    a2 = a['a2']
    ...
    b1 = b['b1']
    b2 = b['b2']
    x = (X - ((a2/b2)*Y) + ((b0/b2)*a2) -a0)/(a1-(a2*b1/b2))
    y = (Y - b1*x -b0)/b2
    
#     y = (Y - b0 -((b1/a1)*X) + ((b1/a1)*a0)) / (b2 + ((b1/a1)*a2))
#     x = (X - a0 -a2*y )/a1
    return x,y
...
def fitGCPfile(path):
    """
    Perform all the fitting process.
    """
    #Read file
#     path = '../empirical_transformations/gcp.txt'
    F = open(path,'r')
    rawD = F.readlines()
    F.close()
    points = []
    ...
    print "Calculating geodetic transformation parameters"
